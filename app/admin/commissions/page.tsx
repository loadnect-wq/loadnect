...
import {
  fetchAllCommissions, fetchOwnerOptions, fetchSettlementAdjustments,
} from "@/lib/admin";
...
const FILTERS = [
  ...
  { key: "paid",       label: "Settled",    value: "paid"      },
  { key: "refunded",   label: "Refunded",   value: "refunded"  },
];

// Ledger states under the single commission model. The owner-billed states
// (pending / overdue / payment_submitted / rejected) can no longer be produced —
// createCommission writes 'collected' and payout settlement writes 'paid' — but
// historical rows are never rewritten, so they still render honestly if present.
const STATUS_CFG: Record<string, { label: string; variant: BadgeVar }> = {
  collected: { label: "Retained from advance", variant: "success"   },
  paid:      { label: "Settled at payout",     variant: "success"   },
...
export default async function AdminCommissionsPage({ searchParams }: Props) {
  ...
  const [commissions, ownerOptions, adjustments] = await Promise.all([
    fetchAllCommissions({
